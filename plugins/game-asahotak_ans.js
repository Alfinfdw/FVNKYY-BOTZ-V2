...
handler.before = async function (m) {
    ...
    if (m.quoted.id == this.asahotak[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            await this.sendBut(m.chat, `*Benar!* +${this.asahotak[id][2]} XP`, 'By Andika', 'Asah Otak', '.asahotak', m)
            clearTimeout(this.asahotak[id][3])
            delete this.asahotak[id]
        } else if (similarity(m.text.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold) m.reply(`*Dikit Lagii >-<*`)
        else m.reply(`*Salah!😜*`)
    }
    return !0
}
