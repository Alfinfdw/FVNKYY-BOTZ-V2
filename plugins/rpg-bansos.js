  ...
  try {
    ...
    if (new Date - global.db.data.users[m.sender].lastbansos > 300000) {
      if (Aku > Kamu) {
        ...
        conn.sendBut( m.chat, `Sorry Gan Lu ga Berhasil Korupsi bansos Dan Tidak masuk penjara karna kamu *melarikan diri🏃*`, `${botol}`, `Kembali`, `${usedPrefix}menu`, m)
        global.db.data.users[m.sender].lastbansos = new Date * 1
      }
    } else conn.sendBut(m.chat, `Kamu sudah Melakukan Korupsi Bansos 💰\nDan kamu harus menunggu agar bisa korupsi bansos kembali \n▸ 🕓 *${timers}*`, `${botol}`, `⋮☰ Mᴇɴᴜ`, `${usedPrefix}menu`, m)
  } catch (e) {
    throw `${e}`
  }
}
