...
`.trim(), link_creator)
    ...
    let share_text = `
Dapatkan ${xp_first_time} XP bagi yang menggunakan link/kode referal dibawah ini.
...
atau bisa juga kirim pesan ini kepada teman kamu🐼

${Object.entries(xp_bonus).map(([count, xp]) => `${count} Orang = Bonus ${xp} XP`).join('\n')}
`.trim())
  }
}
handler.help = ['referal']
handler.tags = ['main']

handler.command = /^(ref|referal|referall)$/i
